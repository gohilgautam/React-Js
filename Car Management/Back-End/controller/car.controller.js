...
const { hasCloudinaryCredentials } = require("../config/cloudinary.config");

exports.addcar = async (req, res) => {
  try {
    if (req.file) {
      if (hasCloudinaryCredentials) {
        // Cloudinary path is available
    ...
    return res.status(500).json({ msg: "Something went wrong...", error: err.message || err });
  }
}
